...
describe("carousel renderer smoke coverage", () => {
  ...
  it("renders a high-activity 8-slide photo-first fixture into expected PNG count", { timeout: PLAYWRIGHT_TEST_TIMEOUT_MS }, async () => {
    ...
    // photo-first fast-path copies raw AI-generated bytes directly; dimensions
    // come from the source visual (1024x1280 for OpenAI), not from Playwright.
    // Verify only that output files are valid PNGs (not their dimensions).
...
// verbatim. Dimensions depend on the source visual (1024x1280), not Playwright.
// Only verify the PNG signature; skip dimension assertions.
